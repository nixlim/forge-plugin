...
- Refactor (tests): the class `Revision9BoundCLIIntegrationTests` in `tests/test_revision9_cli_surfaces.py` (5,456 code lines; the class 77 methods on the runtime-loaded base `CLI_FIXTURE_SUPPORT.ForgeCLIFixture`) is decomposed with the refactor-python `decompose` skill (0.1.3) in test shape (bead forge-plugin-kzu0, plan `.refactor/plan-revision9-cli.md`): the 15 helpers move verbatim into the support mixin `tests/_revision9_cli_support.py` (674 code lines) and the 62 tests into five scenario modules in class shape, `tests/test_revision9_cli_surfaces_{dispositions 966, start_locks 803, replay 666, multicell_stack 588, ingest 570}.py`, each class inheriting the mixin and the original fixture base, with both collectors' test IDs mapped 1:1 (`.refactor/idmap-revision9-cli-*.json`); the source keeps its eight other classes and a `pass`-only shell of the big class (1,229 code lines, pinned; follow-up forge-plugin-by61). One hand-written preparation commit precedes the moves (the preamble loader block `ROOT`, `CLI_PATH`, `ENVELOPE_KEYS`, the `tests._cli_loader` import, `CLI`, `CORE`, `RUNTIME`, `ENGINE`, `CANDIDATE`, `CLI_FIXTURE_SUPPORT` and `key` relocated verbatim into `tests/_revision9_cli_constants.py`; `load_script`/`package_module` cache in `sys.modules`, so each module is still loaded once), every mover call passes `--import-root "$PWD"`, and the mover prunes the source imports each family leaves unused. The two operator rulings of 2026-09-23 ride in the config prep as the same hunks lane B1 carries (`[tool.ruff.lint.isort] known-local-folder` for the `scripts/`-resident packages; `.refactor/` carved out of `tests/test_migration.py`'s legacy-runtime-name scan). The temporary ruff globs are replaced by measured per-module entries and the six new modules are pinned in `.refactor-baseline.json` at their measured sizes; `tests.test_cli_loader` stayed green at every cluster; no production file changed, no mint.
