...
- Refactor (app): the `MergeEngine` class in `scripts/forge/forge_cli/app/_merge_engine.py` (10,302 code lines, 93 methods) is decomposed with the refactor-python `decompose` skill (0.1.2) in function shape (bead forge-plugin-37fr): method bodies move verbatim to `app/_engine_*.py` seam modules and the class keeps same-named class-body bindings, so every `MergeEngine.<name>` attribute, patch target and CLI behaviour is unchanged. Two prep commits precede the moves: a temporary `app/_engine_*.py` ruff per-file-ignores glob carrying the `_merge_engine.py` codes minus `I001` (relocated grandfathering, replaced by measured per-module entries at finalize), and a type-only correction of the `_recording_common_lock` return annotation from `Iterable` to `Iterator`, which removes seven grandfathered mypy errors (type baseline 241 to 234).
