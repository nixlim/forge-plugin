...
- Refactor (engine): the `Engine` class in `scripts/forge/forge_cli/engine/_engine.py` (3,574 code lines, 41 methods) is decomposed with the refactor-python `decompose` skill in function shape: verb method bodies move verbatim (LibCST, manifest-verified AST oracle, `--strict-bodies`) into `engine/_verbs_*.py` modules and the class keeps one `name = _verbs_<x>.name` binding per moved method, re-wrapped with `staticmethod` or `_serialize_worktree_command` exactly as before, so `Engine.<verb>` lookups, unbound `Engine._profiles_for_path`/`_parse_verdict` calls and `patch.object(Engine, ...)` sites are unchanged; no body, verb, diagnostic or reason-code byte changes (bead forge-plugin-321p, plan `.refactor/plan-engine-class.md`). One config-only prep commit rides ahead of the moves: a temporary `pyproject.toml` ruff per-file-ignores glob for `engine/_verbs_*.py` carrying the `_engine.py` entry's already-grandfathered codes minus `I001` (relocated grandfathering per CLAUDE.md, nothing new forgiven; `I001` is left out because a per-file ignore of it silences the mover's manifested `--format-imports` sort on the destination), replaced at finalize by one measured entry per verb module and a shrunk `_engine.py` entry.
