...
- **SC-017**: Two disjoint runs may proceed concurrently, an overlapping request is refused with both run IDs, a foreign live journal owner cannot append, and the real two-commit-chain/two-merge harness loses no marker, journal identity, or telemetry row and never fails open.
...
| FR-010..FR-015 | Vendored engine | Upstream-compatible validation (Happy), Disjoint run admitted/overlap refused (Edge) | Integration: vendored suite, migrated doc-contract, registry admission; Unit: baseline validate |
