...
- `scripts/forge/{check-halt.sh,acquire-commit-lock.sh,release-commit-lock.sh,run-evals.sh,invariant-guard.sh,configure-dcg.sh,drift-check.sh,drift-staleness.sh,aggregate-telemetry.sh,commit-guard.sh,install.sh}` and `scripts/forge/{check-test-quality.py,archive-run.py,audit-commitments.py,emit-decision-event.py}` — executable governance script surfaces
...
- **FR-111** (MUST): `agents/review-final.md` MUST carry: `model: opus`, `effort: high`, tools limited to `Read, Bash, Glob, Grep, LS`, the upstream read-only-execution paragraph verbatim, the blind-spot compensation clause, and the constitution path `${CLAUDE_PLUGIN_ROOT}/rules/review-constitution.md`.
