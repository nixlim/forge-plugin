...
| `tests/test_docs_contract.py` | 420 | 24 instruction-presence and textual-consistency checks over selected prose and examples: no `commands/*.md` stubs; the close sequence string lives only in the workflow skill; README diagram + usage example; journal-is-not-evidence framing; run init with git baseline and local exclude; execution records worktree/ref before launch; resumed agent uses the next execution dir; uniqueness + successor-run guidance matches runtime; benchmark tag wording; validation documented as an omission check; review context isolation; plain `codex exec` with an exact-SHA prompt; consensus by evidence not agent count; GPU gating commands; task outcome definitions; worktree re-verification in the target; replay dir documented as a generated scaffold; risk-scaled review effort; skill ownership split; absence of removed `event_source: "ide"` / `mode: "observe"` / `codex://threads/` workflows; no `$CODEX` override; and that **only ` ```jsonl ` fences** contain journal examples and each such line parses as exactly one entry. These checks show that required text is present; they do not show that a model follows it. |
...
| `tests/test_prompt_first_workflow.py` | 230 | A scripted replay/plumbing integration test, not live agent or reviewer validation — see below. |
...
- Selected documentation wording is pinned by 24 instruction-presence and consistency assertions in `test_docs_contract.py`, so prose edits in a merge can require corresponding assertion updates. Passing those checks does not show that a model follows the instructions.
