export const meta = {
  name: 'split-app',
  description: 'Split forge_cli.app into a package from an approved plan: package conversion, per-wave rope extraction in worktrees, gated merges with FR-230 re-mint, Codex + Fable review, finalize.',
  phases: [
    { title: 'Package', detail: 'by hand (args.packageDone): git mv to app/__init__.py, baseline re-key, ruff ignores, re-mint, gate, commit' },
...
const SNAP = '.refactor/before-app.json'
...
const REMINT =
  ...
  `prod = [x for x in m["subjects"]["production"] if not x.startswith("${PKG}/app")]\n` +
  `files = subprocess.run(["git", "ls-files", "${PKG}/app"], capture_output=True, text=True, check=True).stdout.split()\n` +
...
const RULES =
  ...
  `abstractions; never Read the source module whole; never touch tests/, cli.py, engine/, chain_core/, fresh_evals.py, candidate.py, .refactor-baseline.json, pyproject.toml ` +
  `or the FR-230 manifest; one cluster per commit. rope_move.py --apply already restores bare names in the source ` +
  `(it prints 'source: restored N bare reference(s)') and repairs the destination's 'from forge_cli.app import' ` +
  `line; do NOT hand-edit bodies or references afterwards. After 'ruff check --fix --select I,F401 <src> <dst>', if the ` +
  `destination still imports a name from the package root 'forge_cli.app' (not from a submodule), that name is ` +
...
const pkg = args.packageDone ? { gate: 'PASS', commit: String(args.packageDone) } : await agent(
  ...
  `3. Gate: ${GATE}\n` +
  `4. If every check passed: git add -A -- ${PKG}/app .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py && git commit -m "${plan.preWave.commit}" (also stage the removed ${PKG}/app.py path). ` +
...
for (let w = 0; w < plan.waves.length; w++) {
  ...
  // args.sequential: every cluster rewrites app/__init__.py, so parallel extraction only ever lands its first
  ...
  const COMMIT_PATHS = `${PKG}/app .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py`
  const commitMsg = `refactor(app): re-exports and re-mint after wave ${w} (${wave.map((c) => c.dest.replace(/^.*\//, '')).join(', ')})`
  const prep = await agent(
    ...
    `(one line per symbol or grouped per target), so that every name in __all__ and every attribute read by cli.py and tests still resolves (tests/test_mutation_runner.py reads app._MUTATION_JOURNAL_SIDEBAND_PREFIX on the root); keep __all__ verbatim; do not add anything else.\n` +
...
// The finalize wave (wave 6 of the plan: trigger tables, spec line 117, CON-02 seam-loop move, baseline, import
// contract, re-mint) is orchestrator work; the run returns here unless args.runReview relaunches it for the reviews.
if (!args.runReview) return { branch: BRANCH, status: 'waves-done', wavesCompleted: completed, waveReports }
...
const codex = args.codexDone ? args.codexDone : await agent(
  `On branch ${BRANCH}, run: ${GATE_ENV}; bash ${S}/codex_review.sh --base ${BASE} --plan ${PLAN_MD} --out .refactor/codex-review-app.md\n` +
...
// ---------------------------------------------------------------- Finalize
phase('Finalize')
const fin = args.finalizeDone ? `finalize done by the orchestrator at ${args.finalizeDone}` : await agent(
  `On branch ${BRANCH} at the repository root, finalize the split (operator-approved items only):\n` +
  `1. docs/specs/forge-plugin-spec.md: rewrite the one sentence in the section-5 bullet that describes the forge_cli package's files so it names the app package (${PKG}/app/, root __init__.py holding __all__ and re-exports, submodules per ${PLAN_MD}) instead of app.py; change nothing else. Then run ${GATE_ENV}; python3 -m unittest tests.test_docs_contract tests.test_repo_conformance and update the docs-contract pin only if that test names the sentence.\n` +
  `2. .refactor-baseline.json: run ${GATE_ENV}; git ls-files -z '*.py' | xargs -0 python3 scripts/check_file_length.py --write-baseline .refactor-baseline.json --max 500, then verify the ONLY new entries are files under ${PKG}/app/ and every other entry is unchanged or removed (the old app.py entry disappears); report the new entries with their sizes.\n` +
  `3. pyproject.toml [tool.importlinter]: add one 'layers' contract for forge_cli.app in wave order from ${PLAN_MD} (top: _dispatch; then _merge_engine; bottom: the three wave-0 leaves), no ignore_imports; run ${GATE_ENV}; lint-imports.\n` +
  ...
  `7. Full gate-1: ${GATE1}\n` +
  `8. If all passed: git add docs/specs/forge-plugin-spec.md .refactor-baseline.json pyproject.toml .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py tests/test_docs_contract.py && git commit -m "refactor(app): finalize split (spec sentence, baseline, import contract)"\n` +
  `Return a Markdown report: table of ${PKG}/app/*.py with code-line counts (python3 scripts/check_file_length.py --max 0 on each), symbols moved, gate status of steps 4-7, Codex verdict ${JSON.stringify(codex && codex.verdict)} (the Fable review of the finalized tree and the consensus round follow in the run result), the tip SHA, and the exact commands to reproduce the gate.`,
...
const review = await agent(
  `Review the FINALIZED split of forge_cli.app on branch ${BRANCH} (finalize has already committed the spec sentence, the measured size baseline and the import contract): baseline commit ${BASE}, HEAD, plan ${PLAN_MD}, critique .refactor/critique-app.md, snapshot ${SNAP}, scripts ${S}, ` +
  ...
  `grep for raw patch.object sites on app aliases (APP, CLI.app, forge_cli.app) in tests/ outside patch_app, the ONE non-move edit (E0 patch_app helper and its six call sites; there is no E1 and no module-level statement to relocate), import-time side effects, byte-identical reason literals), then adjudicate every Codex BLOCKING finding as CONFIRMED/REFUTED/UNVERIFIABLE with evidence. ` +
