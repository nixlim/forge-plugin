export const meta = {
  name: 'split-engine',
  description: 'Split forge_cli.engine into a package from an approved plan: package conversion, per-wave rope extraction in worktrees, gated merges with FR-230 re-mint, Codex + Fable review, finalize.',
  phases: [
    { title: 'Package', detail: 'by hand (args.packageDone): git mv to engine/__init__.py, baseline re-key, ruff ignores, re-mint, gate, commit' },
...
const SNAP = '.refactor/before-engine.json'
...
const REMINT =
  ...
  `prod = [x for x in m["subjects"]["production"] if not x.startswith("${PKG}/engine")]\n` +
  `files = subprocess.run(["git", "ls-files", "${PKG}/engine"], capture_output=True, text=True, check=True).stdout.split()\n` +
...
const RULES =
  ...
  `abstractions; never Read the source module whole; never touch tests/, app.py, cli.py, chain_core/, .refactor-baseline.json, pyproject.toml ` +
  `or the FR-230 manifest; one cluster per commit. rope_move.py --apply already restores bare names in the source ` +
  `(it prints 'source: restored N bare reference(s)') and repairs the destination's 'from forge_cli.engine import' ` +
  `line; do NOT hand-edit bodies or references afterwards. After 'ruff check --fix --select I,F401 <src> <dst>', if the ` +
  `destination still imports a name from the package root 'forge_cli.engine' (not from a submodule), that name is ` +
...
const pkg = args.packageDone ? { gate: 'PASS', commit: String(args.packageDone) } : await agent(
  ...
  `3. Gate: ${GATE}\n` +
  `4. If every check passed: git add -A -- ${PKG}/engine .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py && git commit -m "${plan.preWave.commit}" (also stage the removed ${PKG}/engine.py path). ` +
...
for (let w = 0; w < plan.waves.length; w++) {
  ...
  // args.sequential: every cluster rewrites engine/__init__.py, so parallel extraction only ever lands its first
  ...
  const COMMIT_PATHS = `${PKG}/engine .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py`
  const commitMsg = `refactor(engine): re-exports and re-mint after wave ${w} (${wave.map((c) => c.dest.replace(/^.*\//, '')).join(', ')})`
  const prep = await agent(
    ...
    `(one line per symbol or grouped per target), so that every name in __all__ and every attribute read by app.py, cli.py, chain_core and tests still resolves; if the wave moved _build_chain_journal_records, confirm python3 -c 'import forge_cli.engine, forge_cli.runtime as r; assert r._build_chain_journal_records is forge_cli.engine._build_chain_journal_records'; keep __all__ verbatim; do not add anything else.\n` +
...
const codex = args.codexDone ? args.codexDone : await agent(
  `On branch ${BRANCH}, run: ${GATE_ENV}; bash ${S}/codex_review.sh --base ${BASE} --plan ${PLAN_MD} --out .refactor/codex-review-engine.md\n` +
...
const fin = args.finalizeDone ? `finalize done by the orchestrator at ${args.finalizeDone}` : await agent(
  ...
  `1. docs/specs/forge-plugin-spec.md: rewrite the one sentence in the section-5 bullet that describes the forge_cli package's files so it names the engine package (${PKG}/engine/, root __init__.py holding __all__ and re-exports, submodules per ${PLAN_MD}) instead of engine.py; change nothing else. Then run ${GATE_ENV}; python3 -m unittest tests.test_docs_contract tests.test_repo_conformance and update the docs-contract pin only if that test names the sentence.\n` +
  `2. .refactor-baseline.json: run ${GATE_ENV}; git ls-files -z '*.py' | xargs -0 python3 scripts/check_file_length.py --write-baseline .refactor-baseline.json --max 500, then verify the ONLY new entries are files under ${PKG}/engine/ and every other entry is unchanged or removed (the old engine.py entry disappears); report the new entries with their sizes.\n` +
  `3. pyproject.toml [tool.importlinter]: add one 'layers' contract for forge_cli.engine in wave order from ${PLAN_MD} (top: the last-wave modules; bottom: _state), no ignore_imports; run ${GATE_ENV}; lint-imports.\n` +
  ...
  `7. Full gate-1: ${GATE1}\n` +
  `8. If all passed: git add docs/specs/forge-plugin-spec.md .refactor-baseline.json pyproject.toml .forge/evals/tasks tests/fixtures/fr230-results tests/test_fr223_v2_byte_pins.py tests/test_docs_contract.py && git commit -m "refactor(engine): finalize split (spec sentence, baseline, import contract)"\n` +
  `Return a Markdown report: table of ${PKG}/engine/*.py with code-line counts (python3 scripts/check_file_length.py --max 0 on each), symbols moved, gate status of steps 4-7, Codex verdict ${JSON.stringify(codex && codex.verdict)} (the Fable review of the finalized tree and the consensus round follow in the run result), the tip SHA, and the exact commands to reproduce the gate.`,
...
const review = await agent(
  `Review the FINALIZED split of forge_cli.engine on branch ${BRANCH} (finalize has already committed the spec sentence, the measured size baseline and the import contract): baseline commit ${BASE}, HEAD, plan ${PLAN_MD}, critique .refactor/critique-engine.md, snapshot ${SNAP}, scripts ${S}, ` +
  ...
  `grep for raw patch.object sites on engine aliases in tests/ outside patch_engine, the three non-move edits (E0 patch helper, E1 re-exec target, the seam-marker loop move), import-time side effects, byte-identical reason literals), then adjudicate every Codex BLOCKING finding as CONFIRMED/REFUTED/UNVERIFIABLE with evidence. ` +
