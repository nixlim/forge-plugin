...
out=${GATE_OUT:-.refactor/gate-merge-engine-wave-close.txt}
...
export REFACTOR_TEST_CMD="bash .refactor/merge-engine-full-set.sh .refactor/${FULL_TAG:-fullset-merge-engine-wave-close}"
unset FORGE_SESSION_PID REFACTOR_TYPE_CMD
{
  ...
  echo "verify_rc=$?"
  grep "full set:" .refactor/${FULL_TAG:-fullset-merge-engine-wave-close}/*.txt 2>/dev/null; tail -2 .refactor-gate.log | head -1
...
echo done > "$TMPDIR/${FULL_TAG:-gate-merge-engine-wave-close}.done"
