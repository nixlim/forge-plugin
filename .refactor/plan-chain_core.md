...
### _controls.py — Record-key sets, control tuples and record schemas: the `COMMON_LOCK_*` kinds/operations/keys, `_CHAIN_LEASE_KEYS`, every `_REQUIRED_*_CONTROLS` tuple with its public alias, `INGEST_PROOF_ORDER`, the chain-tombstone schema/event/keys and the `_MERGE_CLEANUP_*`/`_EPOCH_*`/`_MERGE_CANDIDATE_OBSERVATION_*`/`_BOOTSTRAP_*` schema strings and fence-operation map.
- symbols (dependency order): COMMON_LOCK_OWNER_KINDS, COMMON_LOCK_OPERATIONS, COMMON_LOCK_FENCE_OPERATIONS, COMMON_LOCK_RECOVERY_KINDS, _COMMON_LOCK_OWNER_KEYS, _COMMON_LOCK_FENCE_KEYS, _COMMON_LOCK_RECOVERY_KEYS, _CHAIN_LEASE_KEYS, _REQUIRED_COMMON_LOCK_CONTROLS, COMMON_LOCK_CONTROLS, CHAIN_TOMBSTONE_SCHEMA, CHAIN_TOMBSTONE_EVENT, CHAIN_TOMBSTONE_KEYS, _REQUIRED_MERGE_STORE_CONTROLS, MERGE_STORE_CONTROLS, _REQUIRED_MERGE_ADAPTER_CONTROLS, MERGE_ADAPTER_CONTROLS, _REQUIRED_MERGE_INTEGRATION_CONTROLS, MERGE_INTEGRATION_CONTROLS, INGEST_PROOF_ORDER, _REQUIRED_INGEST_PROOF_CONTROLS, INGEST_PROOF_CONTROLS, _MERGE_CLEANUP_INTENT_SCHEMA, _MERGE_CLEANUP_RESULT_SCHEMA, _MERGE_CLEANUP_CLOSE_SCHEMA, _MERGE_CLEANUP_RECOVERY_SCHEMA, _MERGE_CLEANUP_FENCE_OPERATIONS, _EPOCH_FETCH_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_EVIDENCE_SCHEMA, _BOOTSTRAP_FETCH_OBSERVATION_SCHEMA
- owns state: COMMON_LOCK_OWNER_KINDS, COMMON_LOCK_OPERATIONS, COMMON_LOCK_FENCE_OPERATIONS, COMMON_LOCK_RECOVERY_KINDS, _COMMON_LOCK_OWNER_KEYS, _COMMON_LOCK_FENCE_KEYS, _COMMON_LOCK_RECOVERY_KEYS, _CHAIN_LEASE_KEYS, _REQUIRED_COMMON_LOCK_CONTROLS, COMMON_LOCK_CONTROLS, CHAIN_TOMBSTONE_SCHEMA, CHAIN_TOMBSTONE_EVENT, CHAIN_TOMBSTONE_KEYS, _REQUIRED_MERGE_STORE_CONTROLS, MERGE_STORE_CONTROLS, _REQUIRED_MERGE_ADAPTER_CONTROLS, MERGE_ADAPTER_CONTROLS, _REQUIRED_MERGE_INTEGRATION_CONTROLS, MERGE_INTEGRATION_CONTROLS, INGEST_PROOF_ORDER, _REQUIRED_INGEST_PROOF_CONTROLS, INGEST_PROOF_CONTROLS, _MERGE_CLEANUP_INTENT_SCHEMA, _MERGE_CLEANUP_RESULT_SCHEMA, _MERGE_CLEANUP_CLOSE_SCHEMA, _MERGE_CLEANUP_RECOVERY_SCHEMA, _MERGE_CLEANUP_FENCE_OPERATIONS, _EPOCH_FETCH_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_EVIDENCE_SCHEMA, _BOOTSTRAP_FETCH_OBSERVATION_SCHEMA
- estimated code lines: 197
- imports from package: none (leaf)
- wave: 0
- cycle risks and resolution: none: leaf (wave 0 step 2; no edge to `_state`). Alias assignments stay adjacent to their `_REQUIRED_*` source here.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): common-lock/lease key sets vs control tuples + schemas

### _state.py — Module-level constants and process state: schema/kind/gate names, `STATES`/`STATE_KEYS`/`EVENT_KEYS`, the `MERGE_*` key/name sets, `TIER_RANK`, timing/cap constants, `ZERO_DIGEST`, `COMMON_LOCK_*_NAME`s, the four compiled regexes, `_MERGE_INACTIVE_*`, `_MERGE_REMOTE_ONLY_IDENTITY_FIELDS`, `_MERGE_SCOPE_UNSET/OVERLAY`, and the three worktree-lock dicts with their sole accessor `_exclusive_descriptor_lock`; imports only `forge_cli.runtime` (for `FENCED_CHILD_DRAIN_CAP_BYTES`).
- symbols (dependency order): SCHEMA, KIND, FRESH_REVIEWER_EVALS_GATE, FRESH_REVIEWER_EVALS_REQUESTS, FRESH_REVIEWER_EVALS_REQUESTED_EVENT, STATES, STATE_KEYS, EVENT_KEYS, MERGE_STATE_KEYS, _MERGE_INACTIVE_ATTEMPT_OBSERVATION_SOURCES, _MERGE_INACTIVE_POST_ATTEMPT_RECOVERY_SOURCES, MERGE_EVENT_KEYS, MERGE_EVENT_NAMES, MERGE_CONSEQUENTIAL_EVENTS, TIER_RANK, INACTIVE_SECONDS, FENCED_CHILD_ACK_TIMEOUT_SECONDS, FENCED_CHILD_DRAIN_SECONDS, FENCED_CHILD_DRAIN_CAP_BYTES, FENCED_CHILD_STOP_GRACE_SECONDS, FENCED_CHILD_REAP_SECONDS, ZERO_DIGEST, COMMON_LOCK_TIMEOUT_SECONDS, COMMON_LOCK_POLL_SECONDS, COMMON_LOCK_RECORD_CAP_BYTES, MERGE_SCOPE_BINDING_CAP_BYTES, COMMON_LOCK_INTENT_NAME, COMMON_LOCK_DIRECTORY_NAME, COMMON_LOCK_OWNER_NAME, COMMON_LOCK_FLOCK_NAME, COMMON_LOCK_RECOVERY_NAME, COMMON_LOCK_INFLIGHT_NAME, CHAIN_ID_RE, SHA256_RE, COMMIT_RE, RUN_ID_RE, _WORKTREE_LOCKS_GUARD, _WORKTREE_LOCKS, _WORKTREE_LOCK_STATE, _exclusive_descriptor_lock, _MERGE_REMOTE_ONLY_IDENTITY_FIELDS, _MERGE_SCOPE_UNSET, _MERGE_SCOPE_OVERLAY
- owns state: SCHEMA, KIND, FRESH_REVIEWER_EVALS_GATE, FRESH_REVIEWER_EVALS_REQUESTS, FRESH_REVIEWER_EVALS_REQUESTED_EVENT, STATES, STATE_KEYS, EVENT_KEYS, MERGE_STATE_KEYS, _MERGE_INACTIVE_ATTEMPT_OBSERVATION_SOURCES, _MERGE_INACTIVE_POST_ATTEMPT_RECOVERY_SOURCES, MERGE_EVENT_KEYS, MERGE_EVENT_NAMES, MERGE_CONSEQUENTIAL_EVENTS, TIER_RANK, INACTIVE_SECONDS, FENCED_CHILD_ACK_TIMEOUT_SECONDS, FENCED_CHILD_DRAIN_SECONDS, FENCED_CHILD_DRAIN_CAP_BYTES, FENCED_CHILD_STOP_GRACE_SECONDS, FENCED_CHILD_REAP_SECONDS, ZERO_DIGEST, COMMON_LOCK_TIMEOUT_SECONDS, COMMON_LOCK_POLL_SECONDS, COMMON_LOCK_RECORD_CAP_BYTES, MERGE_SCOPE_BINDING_CAP_BYTES, COMMON_LOCK_INTENT_NAME, COMMON_LOCK_DIRECTORY_NAME, COMMON_LOCK_OWNER_NAME, COMMON_LOCK_FLOCK_NAME, COMMON_LOCK_RECOVERY_NAME, COMMON_LOCK_INFLIGHT_NAME, CHAIN_ID_RE, SHA256_RE, COMMIT_RE, RUN_ID_RE, _WORKTREE_LOCKS_GUARD, _WORKTREE_LOCKS, _WORKTREE_LOCK_STATE, _MERGE_REMOTE_ONLY_IDENTITY_FIELDS, _MERGE_SCOPE_UNSET, _MERGE_SCOPE_OVERLAY
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): constants (names, timing, keys) vs regexes + worktree locks + `_exclusive_descriptor_lock`
...
- imports from package: _controls, _state
- wave: 0
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): exceptions/dataclasses vs helper functions
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): none needed (~90 lines)
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_ChainReceiptSnapshotVerifier` alone vs lock/snapshot helpers
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_plan_transition_valid` alone vs the six fact helpers
...
- imports from package: _controls, _core, _state
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): record validator vs transition validator
...
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): none available (`validate_state` is one 431-line function; if it crosses 500 with its header it joins the baseline list under operator direction)
...
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): spawn side (`_BlockedFenceChild`, `_pipe_cloexec`, `_read_child_ack`, `_spawn_blocked_fence_child`) vs reap side (`_waitpid_nohang`, `_wait_for_child_exit`, `_terminate_fenced_group`, `_stop_unstarted_child`, `_collect_fenced_child`)

### _ingest_capture.py — Ingest input reading and evidence capture: `_read_ingest_input`, blob/run/record evidence capture.
- symbols (dependency order): _read_ingest_input, _capture_ingest_blob, _capture_run_evidence, _capture_ingest_record_evidence
- owns state: none
- estimated code lines: 302
- imports from package: _core
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_read_ingest_input` alone vs the three capture helpers

### _ingest_currency.py — Ingest currency and live-chain proof: captured-path resolution, step and secret-scan currency, `_prove_ingest_live_chain`.
- symbols (dependency order): _ingest_captured_paths, _ingest_step_is_current, _ingest_secret_scan_is_current, _prove_ingest_live_chain
- owns state: none
- estimated code lines: 180
- imports from package: _core, _state
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`, `_controls`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_prove_ingest_live_chain` alone vs the three currency checks

### _lock_record_validators.py — FR-235 published-record validators: owner, fence, recovery and chain-lease record shape validation.
- symbols (dependency order): _validate_owner_record, _validate_fence_record, _validate_recovery_record, _validate_chain_lease_record
- owns state: none
- estimated code lines: 141
- imports from package: _controls, _core, _state
- wave: 1
- cycle risks and resolution: none: depends on `_controls`, `_state`, `_core`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): owner/fence vs recovery/lease

### _merge_cleanup_intent.py — Merge cleanup intent records: the recovery-intent leaf helpers, expected subject/argv and `_merge_cleanup_intent_valid`.
- symbols (dependency order): _recovery_event_intent, _recovery_cleanup_intent, _merge_cleanup_expected_subject, _merge_cleanup_expected_argv, _merge_cleanup_intent_valid
- owns state: none
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): expected subject/argv vs intent validity + leaf helpers

### _merge_events.py — Merge event reduction: `_merge_event_outbox`, `_merge_payload_delta`, `reduce_merge_event`.
- symbols (dependency order): _merge_event_outbox, _merge_payload_delta, reduce_merge_event
- owns state: none
- estimated code lines: 191
- imports from package: _core, _state
- wave: 1
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_payload_delta` alone vs outbox + reducer
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): rebase/containment vs remote-observation argv builders

### _merge_state_shape.py — Merge-state shape validators: gate plan, epoch, bootstrap classification pending, Revision-9 compatibility view, state/ingest-state shape, additive-grammar history check.
- symbols (dependency order): _merge_gate_plan_valid, _merge_epoch_valid, _merge_bootstrap_classification_pending, _merge_revision9_compatibility_view, _merge_state_shape_valid, _merge_ingest_state_shape_valid, _merge_history_uses_additive_grammar
- owns state: none
- estimated code lines: 289
- imports from package: _core, _state
- wave: 1
- cycle risks and resolution: none: depends on `_core`, `_state`, `_controls`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_bootstrap_classification_pending` + `_merge_revision9_compatibility_view` vs the rest
...
- wave: 1
- cycle risks and resolution: none.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `Repository` alone vs `_committed_changelog_output_paths`

### _lock_record_io.py — Owned-record I/O primitives: read/create/publish/revalidate/unlink of owned records, lock-directory open, opaque path evidence, `_inspect_common_lock_fd`.
- symbols (dependency order): _read_owned_record_at, _same_published_record, _open_lock_directory, _opaque_path_evidence_at, _inspect_common_lock_fd, _create_private_record_at, _publish_no_replace_link, _revalidate_record_at, _unlink_revalidated_record_at, _record_at_if_present
- owns state: none
- estimated code lines: 330
- imports from package: _core, _lock_record_validators, _state
- wave: 2
- cycle risks and resolution: depends on `_lock_record_validators` (hence one wave later than it), `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_inspect_common_lock_fd` + `_create_private_record_at` vs the small record helpers

### _merge_cleanup_history.py — Merge cleanup intent transitions and history: unmatched intent, retry proof, intent transition, git-mutation intent detection, cleanup evidence history and summary.
- symbols (dependency order): _merge_cleanup_evidence_history, _merge_cleanup_history_summary, _merge_cleanup_unmatched_intent, _merge_cleanup_retry_proof_valid, _merge_cleanup_intent_transition_valid, _merge_history_has_git_mutation_intent
- owns state: none
- estimated code lines: 290
- imports from package: _controls, _merge_cleanup_intent
- wave: 2
- cycle risks and resolution: depends on `_merge_cleanup_intent` (`_recovery_cleanup_intent`, `_recovery_event_intent`), hence one wave later; `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): history/summary pair vs intent transition trio
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_cleanup_observation_valid` alone vs the five observation readers
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): fetch-binding validation (`_validate_merge_scope_fetch_binding`, `_merge_scope_binding_names`, `_merge_retained_inflight`, `_merge_scope_binding_validator`) vs environment/request/argv (`_merge_scope_environment_contract`, `_validate_merge_scope_request`, `_merge_scope_argv`, `_merge_full_patch_argv`)
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): progress validation (`_remote_observation_progress_valid`, `_remote_containment_evidence_valid`) vs transition/replay (`_remote_observation_progress_transition_valid`, `_remote_observation_progress_matches_observed`, `_replayed_remote_observation_completed`)

### _merge_candidate_observation_steps.py — Merge candidate-observation step specs, step names and binding.
...
- owns state: none
- estimated code lines: 225
- imports from package: _core, _merge_scope_binding, _state
- wave: 3
- cycle risks and resolution: depends on `_merge_scope_binding` (`_merge_scope_argv`, `_merge_full_patch_argv`), `_core`, `_state`, `_controls`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_candidate_observation_step_specs` alone vs names/binding
...
- imports from package: _controls, _core, _merge_cleanup_intent, _merge_cleanup_observation, _state
- wave: 3
- cycle risks and resolution: depends on `_merge_cleanup_observation`, `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_cleanup_process_result_valid` alone vs step/results/transition
...
- imports from package: _controls, _core, _merge_cleanup_history, _merge_events, _merge_rebase
- wave: 3
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): one function per module
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_scope_transition_valid` alone vs proof/event-binding

### _merge_candidate_observation.py — Merge candidate-observation record/transition validation and evidence extraction.
- symbols (dependency order): _merge_candidate_observation_record_valid, _merge_candidate_observation_transition_valid, _merge_candidate_observation_evidence, _merge_candidate_observation_evidence_valid
- owns state: none
- estimated code lines: 258
- imports from package: _controls, _core, _merge_candidate_observation_steps, _state
- wave: 4
- cycle risks and resolution: depends on `_merge_candidate_observation_steps`, `_core`, `_state`, `_controls`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_candidate_observation_record_valid` alone vs transition/evidence
...
- imports from package: _controls, _core, _merge_candidate_observation, _merge_rebase, _state
- wave: 5
- cycle risks and resolution: depends on `_merge_candidate_observation`, `_merge_rebase`, `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_epoch_fetch_result_intent_digest` alone vs the three record validators
...
- imports from package: _controls, _core, _merge_cleanup_intent, _merge_cleanup_result, _merge_epoch, _state
- wave: 6
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_classify_merge_recovery_lifecycle` alone vs its three helpers
...
- imports from package: _core, _lock_record_validators, _merge_candidate_observation, _merge_rebase, _merge_recovery_lifecycle, _state
- wave: 7
- cycle risks and resolution: depends on `_merge_candidate_observation`, `_merge_rebase`, `_lock_records`, `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_merge_recovery_proof_transition_valid` alone vs the two digest/predecessor helpers
...
- imports from package: _bootstrap_observation, _controls, _core, _merge_candidate_observation, _merge_candidate_observation_steps, _merge_cleanup_history, _merge_cleanup_result, _merge_epoch, _merge_plan, _merge_rebase, _merge_recovery_proof, _merge_release, _merge_scope, _merge_state_shape, _remote_observation, _state
- wave: 8
...
- imports from package: _controls, _core, _ingest_capture, _ingest_currency, _merge_plan, _merge_transition, _repository, _state
- wave: 9
...
- imports from package: _core, _merge_events, _merge_state_shape, _merge_transition, _receipt_snapshot, _state
- wave: 9
- cycle risks and resolution: depends on `_receipt_snapshot`, `_merge_transition`, `_merge_events`, `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_replay_merge_event_bytes` alone vs `validate_merge_state` + `MergeReplayResult`

### _chain_batch_carrier.py — Chain-batch carrier prevalidation, chain-batch target validation, coordination refusal mapping and `_drain_chain_batch_capability` (all leaves).
- symbols (dependency order): _prevalidate_chain_batch_carrier, _coordination_refusal, _validate_chain_batch_target, _drain_chain_batch_capability
- owns state: none
- estimated code lines: 271
- imports from package: none (leaf)
- wave: 10
- cycle risks and resolution: none: leaf (kept in its parent's wave 10).
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_prevalidate_chain_batch_carrier` alone vs the three coordination leaves
...
- imports from package: _core, _merge_replay, _merge_state_shape, _receipt_snapshot, _state
- wave: 10
...
- imports from package: _chain_state, _controls, _core, _merge_replay, _state
- wave: 10
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_require_no_pending_chain_activation_outbox` alone vs projection/preamble

### _chain_batch_authorize.py — The Revision-9 chain-batch authorizer `_authorize_chain_batch`.
- symbols (dependency order): _authorize_chain_batch
- owns state: none
- estimated code lines: 223
- imports from package: _activation, _core
- wave: 11
- cycle risks and resolution: depends on `_activation`, `_core`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): none available (one 227-line function)
...
- imports from package: _activation_outbox, _candidate_v2, _chain_batch_authorize, _chain_batch_carrier, _chain_state, _controls, _core, _ingest_capture, _ingest_currency, _ingest_merge, _merge_events, _merge_state_shape, _repository, _state, _storage
- wave: 12
- cycle risks and resolution: OVER BUDGET (~1,830 code lines): SCC-2 = {ChainStore, CommandContext, _chain_batch_lock, _fresh_reviewer_evals_required, _ingest_proof_verifier, _policy_for_state, _required_steps, _verify_and_build_ingest_records, register_coordination_seams} is a genuine symbol-level cycle (ChainStore -> register_coordination_seams -> _ingest_proof_verifier -> _verify_and_build_ingest_records -> CommandContext -> ChainStore) and must be co-located; no body is edited. Depends on `_storage`, `_chain_batch`, `_activation_outbox`, `_ingest_merge`, `_ingest_capture`, `_candidate_v2`, `_chain_state`, `_repository`, `_merge_events`, `_core`, `_state`. See Follow-up for the later body-edit split.
...
- imports from package: _commit_chain, _core, _lock_record_io, _lock_record_validators, _state
- wave: 13
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): reservation side (`RecoveryReservation`, `_open_owned_directory`, `_publish_recovery_reservation`, `_reservation_evidence`, `_clear_owned_reservation`, `_recovery_record`) vs portable-owner side (`_new_owner_record`, `_fence_matches_owner`, `_publish_portable_owner`, `_release_portable_identity`)
- EXECUTED AS THE FALLBACK SEAM (2026-09-14): the single-module move measured 510 code lines with its header, so wave 13 is two targets, both in wave 13 and both importing only from `_commit_chain`, `_core`, `_lock_record_io`, `_lock_record_validators`, `_state` (no reference between the two sides): `_lock_reservation.py` = reservation side (`_open_owned_directory`, `RecoveryReservation`, `_publish_recovery_reservation`, `_reservation_evidence`, `_clear_owned_reservation`, `_recovery_record`; ~227 code lines) and `_lock_owner.py` = portable-owner side (`_new_owner_record`, `_fence_matches_owner`, `_release_portable_identity`, `_publish_portable_owner`; ~234 code lines).
...
- imports from package: _core, _lock_owner, _lock_record_io, _lock_record_validators, _state
- wave: 14
...
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_recover_stale_portable_owner` alone vs the five helpers
...
- imports from package: _activation_outbox, _chain_batch_carrier, _commit_chain, _core, _ingest_merge, _lock_owner, _lock_record_io, _lock_record_validators, _lock_recovery, _merge_events, _merge_replay, _merge_state_shape, _merge_transition, _state, _storage
- wave: 15
- cycle risks and resolution: OVER BUDGET (~1,900 code lines): SCC-3 = {ChainLease, CommonRebaseLock, MergeChainStore, _clear_reserved_fence, _lease_exclusion_is_current, _lease_reclaim_authority_is_current, _reconcile_merge_projection_for_lease_reclaim, _recovery_classification_receipt_valid, acquire_chain_lease} is a genuine cycle (CommonRebaseLock -> _clear_reserved_fence -> _recovery_classification_receipt_valid -> MergeChainStore -> acquire_chain_lease -> _reconcile... -> CommonRebaseLock) and must be co-located; no body is edited. `_repository_recovery_reservation_present` rides along (used only by `acquire_chain_lease`). Depends on `_lock_recovery`, `_lock_owner`, `_lock_records`, `_commit_chain`, `_chain_batch`, `_activation_outbox`, `_ingest_merge`, `_merge_replay`, `_merge_transition`, `_merge_events`, `_storage`, `_core`, `_state`. See Follow-up.
...
- imports from package: _core, _lock_owner, _lock_record_io, _lock_recovery, _merge_chain, _repository, _state
- wave: 16
...
- imports from package: _controls, _core, _fenced_child, _lock_record_io, _lock_record_validators, _merge_chain, _state
- wave: 16
- cycle risks and resolution: depends on `_merge_chain` (`CommonRebaseLock`), `_fenced_child`, `_lock_records`, `_core`, `_state`.
- fallback seam (orchestrator-only, if the file exceeds 500 code lines with its header): `_publish_fence` vs `run_fenced_command`
...
| `COMMON_LOCK_OWNER_KINDS` | `_controls.py` | `_validate_owner_record` |
| `COMMON_LOCK_OPERATIONS` | `_controls.py` | `_validate_owner_record` |
| `COMMON_LOCK_FENCE_OPERATIONS` | `_controls.py` | `_validate_fence_record`, `run_fenced_command` |
| `COMMON_LOCK_RECOVERY_KINDS` | `_controls.py` | `_validate_recovery_record` |
| `_COMMON_LOCK_OWNER_KEYS` | `_controls.py` | `_validate_owner_record` |
| `_COMMON_LOCK_FENCE_KEYS` | `_controls.py` | `_validate_fence_record` |
| `_COMMON_LOCK_RECOVERY_KEYS` | `_controls.py` | `_validate_recovery_record` |
| `_CHAIN_LEASE_KEYS` | `_controls.py` | `_validate_chain_lease_record` |
| `_REQUIRED_COMMON_LOCK_CONTROLS` | `_controls.py` | `COMMON_LOCK_CONTROLS`, `_require_common_lock_control` |
...
| `CHAIN_TOMBSTONE_KEYS` | `_controls.py` | `_ChainStoragePrimitives` |
| `_REQUIRED_MERGE_STORE_CONTROLS` | `_controls.py` | `MERGE_STORE_CONTROLS`, `_require_merge_store_control` |
| `_REQUIRED_MERGE_ADAPTER_CONTROLS` | `_controls.py` | `MERGE_ADAPTER_CONTROLS`, `_require_merge_adapter_control` |
| `_REQUIRED_MERGE_INTEGRATION_CONTROLS` | `_controls.py` | `MERGE_INTEGRATION_CONTROLS`, `_require_merge_integration_control` |
| `_REQUIRED_INGEST_PROOF_CONTROLS` | `_controls.py` | `INGEST_PROOF_CONTROLS`, `_require_ingest_proof`, `_verify_and_build_ingest_records` |
...
| `_MERGE_CLEANUP_FENCE_OPERATIONS` | `_controls.py` | `_merge_cleanup_intent_valid` |
...
Rules: `_state.py` (step 1) and `_controls.py` (step 2) are moved first, before any user. `_WORKTREE_LOCKS_GUARD`, `_WORKTREE_LOCKS`, `_WORKTREE_LOCK_STATE` are real mutable process state; their only accessor `_exclusive_descriptor_lock` moves with them so no other module touches the dicts. No `global` statements exist (grep). The non-mutable constants are owned by `_state.py` (names, timing, keys, regexes, `_MERGE_REMOTE_ONLY_IDENTITY_FIELDS`, `_MERGE_SCOPE_*`) or `_controls.py` (record-key sets, `_REQUIRED_*` control tuples with their aliases adjacent, `INGEST_PROOF_ORDER`, tombstone constants, record schema strings); the Target modules section lists each explicitly.
...
Size rule: If any non-flagged target exceeds 500 code lines including its header after extraction, the orchestrator (never an extractor) splits it on the seam named in that target's section before the wave is merged; extractors never edit a body or `.refactor-baseline.json`. Flagged (OVER BUDGET) targets are the seven listed in Risk 4 and are recorded in the baseline in the finalize wave.

### Wave 0 (three sequential steps in one worktree; `_core` imports `_state` and `_controls`)
...
- step 2, cluster "controls": target=_controls.py symbols=[COMMON_LOCK_OWNER_KINDS, COMMON_LOCK_OPERATIONS, COMMON_LOCK_FENCE_OPERATIONS, COMMON_LOCK_RECOVERY_KINDS, _COMMON_LOCK_OWNER_KEYS, _COMMON_LOCK_FENCE_KEYS, _COMMON_LOCK_RECOVERY_KEYS, _CHAIN_LEASE_KEYS, _REQUIRED_COMMON_LOCK_CONTROLS, COMMON_LOCK_CONTROLS, CHAIN_TOMBSTONE_SCHEMA, CHAIN_TOMBSTONE_EVENT, CHAIN_TOMBSTONE_KEYS, _REQUIRED_MERGE_STORE_CONTROLS, MERGE_STORE_CONTROLS, _REQUIRED_MERGE_ADAPTER_CONTROLS, MERGE_ADAPTER_CONTROLS, _REQUIRED_MERGE_INTEGRATION_CONTROLS, MERGE_INTEGRATION_CONTROLS, INGEST_PROOF_ORDER, _REQUIRED_INGEST_PROOF_CONTROLS, INGEST_PROOF_CONTROLS, _MERGE_CLEANUP_INTENT_SCHEMA, _MERGE_CLEANUP_RESULT_SCHEMA, _MERGE_CLEANUP_CLOSE_SCHEMA, _MERGE_CLEANUP_RECOVERY_SCHEMA, _MERGE_CLEANUP_FENCE_OPERATIONS, _EPOCH_FETCH_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_SCHEMA, _MERGE_CANDIDATE_OBSERVATION_EVIDENCE_SCHEMA, _BOOTSTRAP_FETCH_OBSERVATION_SCHEMA]
- step 3, cluster "core hubs": target=_core.py symbols=[canonical_bytes, _chain_storage_root, _validated_commitment_path, _parsed_run_captured_path, _require_ingest_proof, iso_z, parse_time, _require_merge_store_control, _require_merge_adapter_control, _require_merge_integration_control, _require_common_lock_control, CommonLockBoundaryCrash, PublishedLockRecord, CommonLockInspection, CommonLockUnavailable, CommonLockReleaseFailure, ChainLeaseUnavailable, FencedChildSurvived, _valid_utc_second, _valid_positive_int, _valid_nonnegative_int, _valid_host, _valid_nonce, _valid_nullable_chain, _write_all, _PublicationCleanupFailure, _process_probe, _group_probe, _sleep_with_deadline, _require_deadline_open, FencedProcessResult, merge_gate_intent_digest, _forge_command, MergeRunTaskSnapshot, _merge_refusal, _valid_sorted_unique_strings]
...
- cluster "ingest capture": target=_ingest_capture.py symbols=[_read_ingest_input, _capture_ingest_blob, _capture_run_evidence, _capture_ingest_record_evidence]
- cluster "ingest currency": target=_ingest_currency.py symbols=[_ingest_captured_paths, _ingest_step_is_current, _ingest_secret_scan_is_current, _prove_ingest_live_chain]
- cluster "lock record validators": target=_lock_record_validators.py symbols=[_validate_owner_record, _validate_fence_record, _validate_recovery_record, _validate_chain_lease_record]
- cluster "merge cleanup intent": target=_merge_cleanup_intent.py symbols=[_recovery_event_intent, _recovery_cleanup_intent, _merge_cleanup_expected_subject, _merge_cleanup_expected_argv, _merge_cleanup_intent_valid]
- cluster "merge events": target=_merge_events.py symbols=[_merge_event_outbox, _merge_payload_delta, reduce_merge_event]
...
- cluster "merge state shape": target=_merge_state_shape.py symbols=[_merge_gate_plan_valid, _merge_epoch_valid, _merge_bootstrap_classification_pending, _merge_revision9_compatibility_view, _merge_state_shape_valid, _merge_ingest_state_shape_valid, _merge_history_uses_additive_grammar]
...
- cluster "lock record io": target=_lock_record_io.py symbols=[_read_owned_record_at, _same_published_record, _open_lock_directory, _opaque_path_evidence_at, _inspect_common_lock_fd, _create_private_record_at, _publish_no_replace_link, _revalidate_record_at, _unlink_revalidated_record_at, _record_at_if_present]
- cluster "merge cleanup history": target=_merge_cleanup_history.py symbols=[_merge_cleanup_evidence_history, _merge_cleanup_history_summary, _merge_cleanup_unmatched_intent, _merge_cleanup_retry_proof_valid, _merge_cleanup_intent_transition_valid, _merge_history_has_git_mutation_intent]
...
- cluster "merge candidate observation steps": target=_merge_candidate_observation_steps.py symbols=[_merge_candidate_observation_step_specs, _merge_candidate_observation_step_names, _merge_candidate_observation_binding]
...
- cluster "merge candidate observation": target=_merge_candidate_observation.py symbols=[_merge_candidate_observation_record_valid, _merge_candidate_observation_transition_valid, _merge_candidate_observation_evidence, _merge_candidate_observation_evidence_valid]
...
### Wave 8 (single cluster; may be chained with adjacent single-cluster waves in one worktree, after wave 7 merged)
...
- cluster "chain batch carrier": target=_chain_batch_carrier.py symbols=[_prevalidate_chain_batch_carrier, _coordination_refusal, _validate_chain_batch_target, _drain_chain_batch_capability]
...
### Wave 11 (parallel, after wave 10 merged)
...
- cluster "chain batch authorize": target=_chain_batch_authorize.py symbols=[_authorize_chain_batch]
...
### Wave 15 (single cluster; may be chained with adjacent single-cluster waves in one worktree, after wave 14 merged)
- cluster "merge chain": target=_merge_chain.py symbols=[_build_merge_chain_journal_records, _new_merge_record_is_current, _prove_merge_run_task_binding, _repository_recovery_reservation_present, MergeChainStore, _recovery_classification_receipt_valid, CommonRebaseLock, _clear_reserved_fence, ChainLease, _lease_exclusion_is_current, _lease_reclaim_authority_is_current, _reconcile_merge_projection_for_lease_reclaim, acquire_chain_lease]

### Wave 16 (parallel, after wave 15 merged)
...
### Wave 17 (sequential, one extractor, orchestrator + operator involvement): finalize
...
6. **Control-class changes bundled in the finalize wave:** the `docs/specs/forge-plugin-spec.md` line 117 sentence (Wave 17) and the `pyproject.toml` import-linter contract. Both require the binding review; the spec edit requires explicit operator approval bound to the reviewed candidate. No other wave touches a control-class path except `tests/**` re-export checks.
...
10. **Wave count.** 17 waves are dependency-forced; single-cluster waves may be chained in one worktree in order. Do not merge multi-cluster waves: a target in wave N imports only wave <N modules, which is what keeps every extractor's diff free of judgment calls.
