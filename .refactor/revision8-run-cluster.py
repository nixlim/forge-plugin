"""Per-cluster driver for the revision8 test split (bead forge-plugin-25ms, lane B1 of
docs/analysis/refactor-overnight-brief-2026-09-23.md).

    python3 .refactor/revision8-run-cluster.py mixin
    python3 .refactor/revision8-run-cluster.py <family> [<family> ...]
...
record the cluster in .refactor/decompose-records-revision8.json and commit the pure mover
...
SRC = "tests/test_revision8_coordination.py"
SRC_MODULE = "test_revision8_coordination"
SRC_CLASS = "Revision8CoordinationTests"
PKG = "tests"
PLAN = ".refactor/plan-revision8.json"
...
def load_cluster(label):
    ...
    if label == "mixin":
        ...
        return {
            ...
            "dest": "tests/_revision8_support.py", "target_class": "Revision8Support",
            "module": "_revision8_support", "test_mode": "identity",
        }
...
def paths_for(label):
    return {
        "manifest": f".refactor/{'mixin' if label == 'mixin' else 'family'}-revision8-{label}.json",
        "snap": f".refactor/revision8-{label}-before.json",
        "ids": f".refactor/tests-revision8-{label}.json",
        "idmap": f".refactor/idmap-revision8-{label}.json",
        "dryrun": f".refactor/dryrun-revision8-{label}.txt",
        "log": f".refactor/apply-revision8-{label}.txt",
        "gate": f".refactor/gate-revision8-{label}.txt",
    }
...
def commit(cluster, p, evidence, dest_lines, src_lines):
    ...
    if cluster["shape"] == "mixin":
        subject = f"refactor(tests): move the {len(cluster['methods'])} revision8 helpers into "
        subject += "tests/_revision8_support.py (mixin shape, tier 1)"
        what = (
            "setUp and every non-test helper of Revision8CoordinationTests moved verbatim by\n"
            "move_methods.py --shape mixin --import-root \"$PWD\" --format-imports; the class\n"
            "header becomes (Revision8Support, unittest.TestCase). Test IDs identical.\n"
        )
    else:
        subject = (
        ...
        what = (
            f"Family {label} of the revision8 split moved verbatim by move_methods.py --shape\n"
    ...
    message = (
        ...
        "and the focused revision8 set PASS (evidence under .refactor/). "
...
def main():
    labels = sys.argv[1:]
    if not labels:
        fail("usage: revision8-run-cluster.py mixin | <family> [<family> ...]")
    for label in labels:
        run_cluster(label)
