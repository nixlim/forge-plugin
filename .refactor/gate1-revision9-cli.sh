...
# forge-project.md into $CELL, for lane B2 of the 2026-09-23 overnight brief (bead
# forge-plugin-kzu0); usage: gate1-revision9-cli.sh <run-label>. Run under the host-wide lock:
#   until [ "$(cut -d. -f1 /proc/loadavg)" -lt 24 ]; do sleep 120; done
#   flock /dev/shm/refactor-gate1.lock bash .refactor/gate1-revision9-cli.sh <label>
set -u
label=${1:?label}
export TMPDIR=/dev/shm/refactor-s6 PYTHONPATH=scripts:scripts/forge
...
} > ".refactor/gate1-revision9-cli-$label.txt" 2>&1
